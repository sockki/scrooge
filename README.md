...
처음 페이지에 들어갈 때, useUser 함수에 의해서 로그인 페이지로 가게 되는데, 이때 페이지 redirect 문제가 발생한 것인지 한번에 로그인이 되지가 않는데, 이것의 정확한 원인을 찾지 못하였음.
